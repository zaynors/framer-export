...
function curl(url, outputPath, referer) {
  ...
  const isMedia = /\.(mp4|webm|mp3|pdf|zip|ogg|mov|mkv)(\?|$)/i.test(url);
  if (isImage || isMedia) {
    const stat = fs.statSync(resolvedPath);
    if (stat.size === 0) {
      fs.unlinkSync(resolvedPath);
      throw new Error(`Empty file for ${url}`);
    }
    // Read 12 bytes to cover MP4's ftyp marker at offset 4
    const readSize = Math.min(12, stat.size);
    const buf = Buffer.alloc(readSize);
    const fd = fs.openSync(resolvedPath, 'r');
    fs.readSync(fd, buf, 0, readSize, 0);
    fs.closeSync(fd);
    const magic = buf.toString('latin1');
    // PNG=\x89PNG, JPEG=\xff\xd8\xff, GIF=GIF8, WebP=RIFF, PDF=%PDF, SVG=<sv
    // MP4=ftyp at offset 4, WebM/MKV=\x1a\x45\xdf\xa3, OGG=OggS, MP3=ID3 or \xff\xfb
    const validMagic = ['\x89PN', '\xff\xd8\xff', 'GIF8', 'RIFF', '%PDF', '<sv', 'OggS', 'ID3', '\xff\xfb', '\xff\xf3', '\xff\xf2'];
    const isMp4 = readSize >= 8 && magic.substring(4, 8) === 'ftyp';
    const isWebM = magic.startsWith('\x1a\x45\xdf\xa3');
    if (!isMp4 && !isWebM && !validMagic.some(m => magic.startsWith(m))) {
      fs.unlinkSync(resolvedPath);
...
function analyzeSite(htmlPath, baseDir) {
  ...
  const jsExtensions = /\.(js|mjs)(\?|$)/i;
  const cssExtensions = /\.css(\?|$)/i;
  const imgExtensions = /\.(png|jpg|jpeg|webp|gif|svg|ico)(\?|$)/i;
  const fontExtensions = /\.(woff2|woff|ttf|otf|eot)(\?|$)/i;

  const jsUrls = allUrls.filter(u => jsExtensions.test(u));
  const cssUrls = allUrls.filter(u => cssExtensions.test(u));
  const imgUrls = allUrls.filter(u => imgExtensions.test(u) || u.includes('framerusercontent.com/images/'));
  const fontUrls = allUrls.filter(u => fontExtensions.test(u) || u.includes('fonts.gstatic') || u.includes('framerusercontent.com/assets/'));
...
function fixHtmlRefs(htmlPath, renameMap, { downloadedJs, downloadedImages, downloadedMedia }, baseDir) {
  ...
  // Support both href="./filename.mjs", href="filename.mjs", and href="js/filename.mjs"
  html = html.replace(/<link rel="modulepreload"[^>]*href="(?:\.\/)?([^"]+\.mjs)"[^>]*>/g, (m, filename) => {
    ...
    return `<link rel="modulepreload" href="/js/${cleanName}">`;
  });
...
function cleanFramerNoiseSingle(htmlFile) {
  ...
  // Remove badge container — match the opening tag and then greedily consume all
  // nested divs by counting open/close tags to handle variable nesting depths
  c = c.replace(/<div id="__framer-badge-container">[\s\S]*?<\/div>[\s]*(?:<\/div>[\s]*)*/g, '');

  // Remove editorbar container — use balanced div matching for nested structures
  c = c.replace(/<div id="__framer-editorbar-container"[^>]*>[\s\S]*?<\/div>[\s]*(?:<\/div>[\s]*)*/g, '');

  // Remove editorbar iframe
  c = c.replace(/<div id="__framer-editorbar"[^>]*>[\s\S]*?<\/div>[\s]*(?:<\/div>[\s]*)*/g, '');
  ...
  if (c !== original) {
    ...
    log(`  Cleaned ${path.relative(path.dirname(htmlFile), htmlFile) || 'index.html'}: removed ${removed} chars`);
  }
}

function cleanFramerNoise(htmlPath, baseDir) {
  logStep(5, 'Removing Framer noise from ALL HTML files...');
  ...
  function findAllHtml(dir) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory() && !['js', 'css', 'images', 'media', 'node_modules', '.git'].includes(entry.name)) {
        findAllHtml(full);
  ...
  // Ensure root htmlPath is included
  if (!allHtmlFiles.includes(htmlPath)) {
    allHtmlFiles.unshift(htmlPath);
  }

  log(`  Processing ${allHtmlFiles.length} HTML files...`);
  allHtmlFiles.forEach(f => cleanFramerNoiseSingle(f));
  log(`  Noise removal complete across ${allHtmlFiles.length} files.`);
}
...
function fixSpaRouting(htmlPath, baseDir, sourceUrl) {
  ...
  // Uses full relative paths so nested routes like /blogs/my-post are correct
  function findHtmlFiles(dir, relativePath) {
    if (!fs.existsSync(dir)) return;
    // Skip asset directories
    const skipDirs = ['js', 'css', 'images', 'media', 'node_modules', '.git'];
    const entries = fs.readdirSync(dir, { withFileTypes: true });
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        if (fs.existsSync(indexPath)) {
          if (!routes.find(r => r.path === `/${relPath}`)) {
            routes.push({ path: `/${relPath}`, file: `${relPath}/index.html` });
          }
          // Also recurse into subdirectories for deeper nesting
          findHtmlFiles(full, relPath);
        } else {
          // No index.html — recurse to find nested .html files
          findHtmlFiles(full, relPath);
        }
      } else if (entry.name.endsWith('.html') && entry.name !== 'index.html') {
        const slug = entry.name.replace(/\.html$/, '');
        const routePath = relativePath ? `/${relativePath}/${slug}` : `/${slug}`;
        const filePath = relativePath ? `${relativePath}/${slug}.html` : `${slug}.html`;
  ...
  allHtmlFiles.forEach(f => {
    ...
    // but skip asset directories (js/, css/, images/, media/) and file extensions
    const assetPrefixes = ['js/', 'css/', 'images/', 'media/'];
    content = content.replace(/href="\.\/([^"]+)"/g, (m, p) => {
      if (p.startsWith('http') || p.startsWith('//') || p.startsWith('#') || p.startsWith('?')) return m;
      // Don't rewrite asset references — they should stay relative
      if (assetPrefixes.some(prefix => p.startsWith(prefix))) return m;
      // Don't rewrite file extensions (e.g. ./logo.png, ./script.js)
      if (/\.[a-z0-9]{2,4}$/i.test(p)) return m;
      modified = true;
...
`);
  ...
  function findAllHtmlPages(dir) {
    ...
    entries.forEach(entry => {
      ...
      if (entry.isDirectory() && !skipDirs.includes(entry.name)) {
        ...
        const indexPath = path.join(full, 'index.html');
        if (fs.existsSync(indexPath) && indexPath !== htmlPath) {
          allHtmlFiles.push(indexPath);
        }
        // Recurse into subdirectories for deeper nesting
        findAllHtmlPages(full);
      } else if (entry.name.endsWith('.html') && full !== htmlPath && !allHtmlFiles.includes(full)) {
        allHtmlFiles.push(full);
      }
    });
  }
  findAllHtmlPages(baseDir);
  ...
  const newJsUrls = allUrlsInPages.filter(u => jsExtensions.test(u) && !analysis.jsUrls.includes(u));
  const newCssUrls = allUrlsInPages.filter(u => cssExtensions.test(u) && !analysis.cssUrls.includes(u));
  const newImgUrls = allUrlsInPages.filter(u => (imgExtensions.test(u) || u.includes('framerusercontent.com/images/')) && !analysis.imgUrls.includes(u));

  const knownTypeSet = new Set([
    ...analysis.jsUrls, ...newJsUrls,
    ...analysis.cssUrls, ...newCssUrls,
    ...analysis.imgUrls, ...newImgUrls,
    ...analysis.fontUrls
  ]);

  const newMediaUrls = allUrlsInPages.filter(u =>
    !knownTypeSet.has(u) && (
      mediaExtensions.test(u) ||
      u.includes('video.framer.com/') ||
      u.includes('framerusercontent.com/files/') ||
      u.includes('framerusercontent.com/sites/')
    ) && !analysis.mediaUrls.includes(u)
  );

  if (newJsUrls.length > 0) {
    log(`Found ${newJsUrls.length} additional JS URLs from downloaded pages`);
    analysis.jsUrls.push(...newJsUrls);
  }
  if (newCssUrls.length > 0) {
    log(`Found ${newCssUrls.length} additional CSS URLs from downloaded pages`);
    analysis.cssUrls.push(...newCssUrls);
  }
  if (newImgUrls.length > 0) {
    log(`Found ${newImgUrls.length} additional Image URLs from downloaded pages`);
    analysis.imgUrls.push(...newImgUrls);
  }
  ...
  // Fix HTML refs and clean Framer noise on ALL HTML files (root + sub-pages)
  const allPageHtmlFiles = [htmlPath];
  function collectAllHtml(dir) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory() && !skipDirs.includes(entry.name)) {
        const indexPath = path.join(full, 'index.html');
        if (fs.existsSync(indexPath) && !allPageHtmlFiles.includes(indexPath)) {
          allPageHtmlFiles.push(indexPath);
        }
        collectAllHtml(full);
      } else if (entry.name.endsWith('.html') && !allPageHtmlFiles.includes(full)) {
        allPageHtmlFiles.push(full);
      }
    }
  }
  collectAllHtml(baseDir);

  log(`Fixing HTML refs across ${allPageHtmlFiles.length} HTML files...`);
  allPageHtmlFiles.forEach(f => {
    fixHtmlRefs(f, renameMap, downloadedAssets, baseDir);
  });
  if (opts.clean) cleanFramerNoise(htmlPath, baseDir);
